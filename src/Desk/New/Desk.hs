...
-- | State which attributes you'd like to memoize: how to call them and what's
-- their type.
...
-- | State that you'd like to be able pattern match on 'C_Position'
-- (@Constructor@ in the original formulation).
type Cxt = WhereAmI C_Position

-- | Not entirely sure this should be a part of the public interface...
type AGZipper = State (Zipper Cxt Attributes Program)

-- | Calculation of the @envi@ attribute.
--
-- First, we declare that the result of this computation should be stored in the
-- "envi" attribute (and no, the names don't have to match).
--
-- The @get >>= \(Zipper hole) -> case whereami hole of@ part is almost
-- equivalent to @constructor >>= \constr -> case const of@ in the original
-- formulation. The difference will become clear later.
...
-- | Calculation of the @name@ attribute.
--
-- Note how after pattern matching on @C_Id@ we can pattern match on @Id@! This
-- is probably __the most important improvement__ as it allows you to get back
-- to your original types and perorm arbitraty actions with them.
...
-- | Calculation of the @value@ attribute
...
-- | Calculation of the @envs@ attribute.
--
-- Yes, I love 'Applicative' :)
...
-- | Calculation of the @ok@ attribute.
ok :: AGZipper Bool
...
-- | Calculation of the @code@ attribute.
...
-- | Notice, there's no @buildMemoTree@ here!
main :: IO ()
main = putStrLn $ fst $ runState code (enter testData)

-- Library magic (i.e. will be generated by TH in the ideal world)
-- -----------------------------------------------------------------------------
